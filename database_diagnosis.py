...
def diagnose_connection():
    """诊断数据库连接问�?""
    print("🔧 LandPPT 数据库连接诊�?)
    print("=" * 50)
    
    # 测试配置
    configs = [
        {
            'name': 'your_db_user (应用用户)',
            'config': {
    ...
    # 如果用户想测�?postgres 用户
    test_postgres = input("是否也测�?postgres 超级用户? (y/N): ").strip().lower()
    if test_postgres in ['y', 'yes']:
        postgres_password = input("请输�?postgres 密码: ").strip()
    ...
    for test_config in configs:
        ...
        print(f"   数据�? {test_config['config']['database']}")
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                print(f"   �?当前数据�? {result[1]}")
                print(f"   �?版本: {result[2][:50]}...")
            
            # 如果�?your_db_user，测试应用相关功�?
            if test_config['config']['user'] == 'your_db_user':
                ...
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    print(f"   �?deployment_verification 表访问正常，�?{count} 条记�?)
    ...
    # 给出建议
    print("\n💡 建议:")
    print("1. 如果 your_db_user 连接失败，请检�?")
    print("   - 用户是否已创�?(运行初始�?SQL)")
    print("   - 密码是否正确")
    print("   - 网络连接是否正常")
    print("\n2. 如果连接成功但功能测试失败，请检�?")
    print("   - landppt schema 是否已创�?)
    print("   - 权限是否正确设置")
    print("   - 表和函数是否已创�?)
