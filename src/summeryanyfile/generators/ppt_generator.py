...
class PPTOutlineGenerator(LoggerMixin):
    ...
    def __init__(
        ...
        self.logger.info(
            f"PPT生成器初始化完成，使用模型: {config.llm_model}，缓存目录: {cache_dir}，处理模式: {processing_mode}"
        )
        ...
        try:
            ...
            chunks = self.document_processor.chunk_document(
                ...
                max_tokens=(
                    ...
                    if self.config.chunk_strategy == ChunkStrategy.FAST
                    else None
                ),
            )
    ...
    async def generate_from_file(
        ...
        try:
            ...
            document_info = await loop.run_in_executor(
                None, self.document_processor.load_document, file_path, encoding
            )
    ...
    def update_config(self, config: ProcessingConfig):
        ...
        # 检查是否需要重新初始化LLM
        if (
            config.llm_model != self.config.llm_model
            or config.llm_provider != self.config.llm_provider
            or config.temperature != self.config.temperature
            or config.max_tokens != self.config.max_tokens
        ):
