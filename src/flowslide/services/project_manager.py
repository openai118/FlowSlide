...
class ProjectManager:
    ...
    def _create_todo_board(
        ...
        return TodoBoard(task_id=project_id, title={request.topic}, stages=stages)

    def update_todo_board_with_confirmed_requirements(
    ...
    async def update_stage_status(
        ...
        for i, stage in enumerate(todo_board.stages):
            if stage.id == stage_id:
                ...
                logger.info(
                    f"Updated stage {stage_id} to {status}, overall progress: {todo_board.overall_progress}%"
                )
