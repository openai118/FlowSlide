"""
File Processing Service for FlowSlide
...
class FileProcessor:
    ...
    def validate_file(
        self, filename: str, file_size: int, max_size_mb: int = 100
    ) -> Tuple[bool, str]:
        ...
        if file_size > max_size_bytes:
            return (
                False,
                f"文件大小超过限制 ({max_size_mb}MB)。当前文件大小: {file_size / 1024 / 1024:.1f}MB",
            )
    ...
    async def create_ppt_from_content(
        self, content: str, suggested_topic: str = None
    ) -> Dict[str, Any]:
