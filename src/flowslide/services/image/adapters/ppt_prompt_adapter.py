...
class PPTSlideContext:
    ...
    language: str = "zh"
    aspect_ratio: str | None = None  # NEW: '16:9' or '4:3'
...
class PPTPromptAdapter:
    ...
    async def generate_image_prompt(self, slide_context: PPTSlideContext) -> str:
        ...
        try:
            ...
            logger.info(
                f"Generated image prompt for slide {slide_context.page_number}: {prompt[:100]}..."
            )
    ...
    def _get_content_description(
        self, slide_context: PPTSlideContext, content_analysis: Dict[str, Any]
    ) -> str:
        """获取内容描述"""
        content_type = content_analysis["type"]
    ...
    def _resolve_aspect_ratio(self, slide_context: PPTSlideContext) -> str:
        """Determine aspect ratio from slide context or global config."""
        # Prefer slide context
        if slide_context.aspect_ratio in ("16:9", "4:3"):
            return slide_context.aspect_ratio
        # Try global config
        try:
    ...
    def _map_sizes_for_provider(self, provider: ImageProvider, aspect_ratio: str) -> dict:
        ...
        if provider == ImageProvider.DALLE:
            # DALL·E 3 supports 1024x1024, 1024x1792, 1792x1024; no perfect 4:3.
            # Use the closest and we will crop/fit on canvas later if needed.
            if aspect_ratio == "4:3":
                # choose portrait-ish then we can letterbox/crop; but prefer landscape. Use 1024x768 equivalent -> pick 1024x1024 and rely on maintain_aspect later.
        ...
        elif provider == ImageProvider.SILICONFLOW:
            if aspect_ratio == "4:3":
                return {"width": 1024, "height": 768}
            return {"width": 1024, "height": 576}
            ...
            return {"width": 1024, "height": 576}
        # default fallback
        return {"width": 1024, "height": 768 if aspect_ratio == "4:3" else 576}

    def _get_generation_params(
        self, slide_context: PPTSlideContext, provider: ImageProvider
    ) -> Dict[str, Any]:
        """获取生成参数"""
        # NEW: resolve aspect and base sizes first
        aspect_ratio = self._resolve_aspect_ratio(slide_context)
        base = self._map_sizes_for_provider(provider, aspect_ratio)
        ...
        if provider == ImageProvider.DALLE:
            params.update(
                {
                    "size": base.get("size", f"{params['width']}x{params['height']}"),
                    "quality": "standard",
                    "style": "natural",
                }
            )
