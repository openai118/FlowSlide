...
class ImageProcessor:
    ...
    def _process_image_sync(
        self, input_path: Path, output_path: Path, options: ImageProcessingOptions
    ) -> ImageOperationResult:
        """同步处理图片"""
        try:
            # 打开图片
            with Image.open(input_path) as img:
                ...
                elif self.auto_resize and (
                    img.width > self.max_width or img.height > self.max_height
                ):
                    # 自动调整尺寸
        ...
        with Image.open(image_path) as img:
            ...
            metadata = ImageMetadata(
                ...
                format=(
                    ...
                    if format_name in [f.value for f in ImageFormat]
    ...
    async def create_thumbnail(
        self, input_path: Path, output_path: Path, size: Tuple[int, int] = (200, 200)
    ) -> ImageOperationResult:
        """创建缩略图"""
        options = ImageProcessingOptions(
            resize_width=size[0],
