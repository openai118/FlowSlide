...
# Application info
app_info = Info(
    "flowslide_app_info", "FlowSlide application information", registry=flowslide_registry
)
...
# User metrics
active_users = Gauge(
    "flowslide_active_users", "Number of currently active users", registry=flowslide_registry
)

user_sessions_total = Counter(
    "flowslide_user_sessions_total", "Total user sessions created", registry=flowslide_registry
)
...
cache_hits_total = Counter(
    "flowslide_cache_hits_total", "Total cache hits", ["cache_type"], registry=flowslide_registry
)
...
class MetricsCollector:
    ...
    def track_ppt_generation(
        ...
        if status == "failed" and error_type:
            ppt_generation_failures_total.labels(scenario=scenario, error_type=error_type).inc()
    ...
    def track_ai_request(
        ...
        if tokens_used:
            for token_type, count in tokens_used.items():
                ai_tokens_used_total.labels(provider=provider, model=model, type=token_type).inc(
                    count
                )
    ...
    def track_file_upload(
        ...
        if status == "failed" and error_type:
            file_upload_failures_total.labels(file_type=file_type, error_type=error_type).inc()

    def track_database_operation(
        self, operation: str, duration: float, error_type: Optional[str] = None
    ):
...
def metrics_endpoint():
    ...
    return PlainTextResponse(
        generate_latest(flowslide_registry), headers={"Content-Type": CONTENT_TYPE_LATEST}
    )
