...
class Project(Base):
    ...
    todo_board: Mapped[Optional["TodoBoard"]] = relationship(
        "TodoBoard", back_populates="project", uselist=False
    )
...
class TodoBoard(Base):
    ...
    project_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("projects.project_id"), unique=True
    )
    ...
    stages: Mapped[List["TodoStage"]] = relationship(
        "TodoStage", back_populates="todo_board", order_by="TodoStage.stage_index"
    )
...
class TodoStage(Base):
    ...
    project_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("projects.project_id"), index=True
    )  # Added for direct project reference
    ...
    )  # Added index for status queries
