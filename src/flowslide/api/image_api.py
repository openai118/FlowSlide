...
async def get_image_service_status(user: User = Depends(get_current_user_required)):
    """获取图片服务状态"""
    try:
...
async def test_image_service(user: User = Depends(get_current_user_required)):
    """测试图片服务"""
    try:
        ...
        test_results = {"providers": {}, "cache_info": {}}
...
async def clear_image_cache(user: User = Depends(get_current_user_required)):
    """清理图片缓存"""
    try:
        image_config = get_image_config()
        config = image_config.get_config()
...
async def generate_image(
    request: ImageGenerationRequest, user: User = Depends(get_current_user_required)
):
    """生成图片"""
    try:
        image_service = get_image_service()
...
async def suggest_images(
    request: ImageSuggestionRequest, user: User = Depends(get_current_user_required)
):
    """为幻灯片建议图片"""
    try:
        image_service = get_image_service()
...
async def get_gallery_stats(user: User = Depends(get_current_user_required)):
    """获取图库统计信息"""
    try:
        image_service = get_image_service()
        ...
        return {"success": True, "stats": cache_stats}
...
async def get_image_info(
    image_id: str, request: Request, user: User = Depends(get_current_user_required)
):
    """获取图片信息，包括绝对URL"""
    try:
        image_service = get_image_service()
...
async def batch_delete_images(
    request: BatchDeleteRequest, user: User = Depends(get_current_user_required)
):
    """批量删除图片"""
    try:
        image_service = get_image_service()
...
async def batch_download_images(
    request: BatchDownloadRequest, user: User = Depends(get_current_user_required)
):
    """批量下载图片"""
    try:
        image_service = get_image_service()
            ...
            try:
                ...
                if image_info and image_info.local_path:
                    ...
                    if image_path.exists():
                        image_infos.append(
                            {"path": str(image_path), "filename": image_info.filename}
                        )
            except Exception as e:
...
async def deduplicate_gallery(user: User = Depends(get_current_user_required)):
    """去重图库中的重复图片"""
    try:
        image_service = get_image_service()
...
async def clear_all_images(user: User = Depends(get_current_user_required)):
    """清空图床 - 删除所有图片"""
    try:
        image_service = get_image_service()
        ...
        if total_images == 0:
            return {"success": True, "deleted_count": 0, "message": "图床已经是空的"}
