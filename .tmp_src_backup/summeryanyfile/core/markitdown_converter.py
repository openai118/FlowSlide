...
class MarkItDownConverter:
        ...
        if enable_cache:
            try:
                ...
                self._cache_manager = FileCacheManager(
                    cache_dir=cache_dir, processing_mode=processing_mode
                )
    ...
    def convert_file(self, file_path: str) -> Tuple[str, str]:
        ...
        if self.enable_cache and self._cache_manager:
            ...
            if is_cached and md5_hash:
                ...
                if cached_content:
                    # 从缓存元数据中获取编码信息
                    encoding = cached_metadata.get("processing_metadata", {}).get(
    ...
    def convert_pdf_url(
        self, pdf_url: str, enable_ocr: bool = True, enable_formula: bool = False
    ) -> Tuple[str, str]:
        """
