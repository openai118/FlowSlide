...
class DatabaseMigration:
    ...
    def _register_migrations(self):
        ...
        # Migration 007: Add owner_id to projects table
        self.migrations.append(
            {
                "version": "007",
                "name": "add_owner_id_to_projects",
                "description": "Add owner_id column to projects table and index it for RBAC",
    ...
    async def _migration_005_down(self, session: AsyncSession):
        ...
        try:
            logger.info(
                "Rolling back migration 005: Removing project_metadata column from projects table"
            )
    ...
    async def _migration_006_down(self, session: AsyncSession):
        ...
        try:
            logger.info(
                "Rolling back migration 006: Removing is_user_edited field from slide_data table"
            )
    ...
    async def _migration_007_up(self, session: AsyncSession):
        """Migration 007: Add owner_id column to projects table and index it"""
        try:
            logger.info("Running migration 007: Adding owner_id to projects table")
    ...
    async def _migration_007_down(self, session: AsyncSession):
        """Migration 007 rollback: Remove owner_id column and index from projects table"""
        try:
            logger.info("Rolling back migration 007: Removing owner_id from projects table")

            # Drop index first
            await session.execute(text("DROP INDEX IF EXISTS idx_projects_owner_id"))

            # SQLite doesn't support DROP COLUMN directly, recreate table without owner_id
            await session.execute(
                text(
                    """
                CREATE TABLE projects_backup AS
                SELECT id, project_id, title, scenario, topic, requirements, status,
                       outline, slides_html, slides_data, confirmed_requirements,
                       project_metadata, version, created_at, updated_at
                FROM projects
            """
                )
            )
