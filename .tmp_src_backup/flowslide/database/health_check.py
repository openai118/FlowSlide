"""
Database health check utilities for FlowSlide
"""
    ...
    async def _check_performance(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            queries = [
                ...
                (
                    "active_stages",
                    "SELECT COUNT(*) FROM todo_stages WHERE status = 'running'",
                ),
            ]
            ...
            slow_queries = [
                name for name, data in metrics.items() if data["response_time_ms"] > 100
            ]
    ...
    async def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                result = await session.execute(
                    text("SELECT COUNT(*) FROM todo_stages WHERE status = 'completed'")
                )
                stats["completed_stages"] = result.scalar()

                result = await session.execute(
                    text("SELECT COUNT(*) FROM todo_stages WHERE status = 'running'")
                )
                ...
                result = await session.execute(
                    text("SELECT COUNT(*) FROM projects WHERE created_at > :yesterday"),
                    {"yesterday": yesterday},
                )
