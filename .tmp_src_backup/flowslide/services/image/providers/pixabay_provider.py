...
class PixabaySearchProvider(ImageSearchProvider):
    ...
    async def search(self, request: ImageSearchRequest) -> ImageSearchResult:
        ...
        try:
            ...
            params = {
                ...
                "page": request.page,
                "per_page": max(
                    3, min(request.per_page, min(self.per_page, 200))
                ),  # API范围：3-200
                "pretty": "false",
            }
    ...
    def _check_rate_limit(self) -> bool:
        ...
        if len(self._request_times) >= self.rate_limit_requests:
            logger.warning(
                f"Pixabay rate limit exceeded: {len(self._request_times)}/{self.rate_limit_requests} in 60 seconds"
            )
            return False
    ...
    def _process_rate_limit_headers(self, headers):
        ...
        try:
            ...
            if rate_limit:
                logger.debug(
                    f"Pixabay rate limit: {rate_remaining}/{rate_limit}, reset in {rate_reset}s"
                )

            # 如果剩余请求数很少，记录警告
            if rate_remaining and int(rate_remaining) < 10:
                logger.warning(
                    f"Pixabay API rate limit nearly exceeded: {rate_remaining} requests remaining"
                )

        except Exception as e:
            logger.debug(f"Failed to process rate limit headers: {e}")

    def _generate_meaningful_filename(
        self, hit: Dict[str, Any], pixabay_id: str, format_ext: str
    ) -> str:
        """生成有意义的文件名"""
        try:
            # 获取标签作为文件名基础
