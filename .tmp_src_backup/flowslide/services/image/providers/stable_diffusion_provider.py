...
class StableDiffusionProvider(ImageGenerationProvider):
    ...
    async def generate(self, request: ImageGenerationRequest) -> ImageOperationResult:
        ...
        try:
            ...
            return ImageOperationResult(
                success=False, message="Request timeout", error_code="timeout"
            )
    ...
    def _prepare_api_request(self, request: ImageGenerationRequest) -> Dict[str, Any]:
        ...
        if request.negative_prompt:
            api_request["text_prompts"].append({"text": request.negative_prompt, "weight": -1.0})

        return api_request

    async def _process_api_response(
        self, response_data: Dict[str, Any], request: ImageGenerationRequest
    ) -> ImageOperationResult:
        """处理API响应"""
        try:
            ...
            # 创建图片信息
            image_info = self._create_image_info(image_path, len(image_data), request, artifact)
    ...
    async def _save_image(self, image_data: bytes, request: ImageGenerationRequest) -> Path:
        """保存生成的图片"""
        # 生成文件名
    ...
    def _generate_tags_from_prompt(self, prompt: str) -> List[ImageTag]:
        ...
        keywords = [
            word.strip(".,!?;:") for word in keywords if word not in stop_words and len(word) > 2
        ]
    ...
    def _extract_keywords_from_prompt(self, prompt: str) -> List[str]:
        ...
        keywords = [
            word.strip(".,!?;:") for word in words if word not in stop_words and len(word) > 2
        ]
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        return {
                            ...
                            "rate_limit_remaining": self.rate_limit_requests
