...
def performance_test():
    ...
    def single_operation(thread_id):
        ...
        try:
            ...
            print(f"   线程 {thread_id}: {duration:.3f}秒")
            return True
    ...
    print("\n" + "=" * 40)
    print("📊 性能分析结果:")
    print(f"   串行平均时间: {sum(serial_times)/len(serial_times):.3f}秒")
    
    if results:
        print(f"   并发平均时间: {sum(results)/len(results):.3f}秒")
        print(f"   并发总时间: {concurrent_duration:.3f}秒")
        print(f"   并发成功率: {success_count}/5 ({success_count/5*100:.0f}%)")
