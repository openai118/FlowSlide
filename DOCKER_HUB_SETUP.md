...
| `docker-build.yml` | 本地构建测试 | 验证Docker构建过程 | ❌ 仅构建测试 | ~3-5分钟 | 每次推送+PR |
...
#### **docker-build.yml (构建验证)**
- 🧪 **构建测试**: 验证Dockerfile和依赖
- ⚡ **快速反馈**: 每次推送都验证构建
- 🔍 **错误检测**: 早期发现构建问题
- ⏱️ **触发条件**: 每次推送和Pull Request

#### **docker-fast-build.yml (开发测试)**
- 🚀 **开发迭代**: develop分支的快速测试
- 🏷️ **开发标签**: 带-dev后缀的测试版本
...
2. 选择相应的工作流程:
   ...
   - **"GitHub Container Registry Build"**: GHCR构建状态
3. 查看构建日志和状态
