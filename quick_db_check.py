...
def quick_database_check():
    ...
    try:
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            # 写入测试
            test_msg = f"快速检查 {datetime.now().strftime('%H:%M:%S')}"
        ...
        print(f"✅ 读写权限正常: 现有 {count_result['count']} 条记录")
    ...
    try:
        ...
        bucket_url = "https://fiuzetazperebuqwmrna.supabase.co/storage/v1/bucket"
        ...
        if response.status_code == 200:
            ...
            if landppt_bucket:
                print(f"✅ 存储桶正常: {landppt_bucket['name']} (public: {landppt_bucket['public']})")
            else:
                print("❌ 未找到 landppt-files 存储桶")
                return False
        else:
